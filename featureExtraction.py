    ...
    while todo_list and changed and counter < max_result:
        ...
        for url in todo_list[:]:
            if counter >= max_result:
                break
            if successfulUpdate(lancaster, stemmed_query, google_result, url, index_to_synset, synset_to_index, noun_to_synset_index, verb_to_synset_index, subsumer_cache):
