...
def jsonFaroo(n, s, q):
    ...
    c = 'web'
    f = 'json'
    k = 'KmC4Y9@EA91Nlzm86Y6FELfF6kA_'
    query = urllib.parse.urlencode({'q':str(q), 'start':str(s), 'length':str(n), 'l':str(l), 'src':str(c), 'f':str(f)})
    url = 'http://www.faroo.com/api?%s&key=%s' % (query, k)
    while True:
...
def jsonGoogle(n, s, q):
    query = urllib.parse.urlencode({'rsz':str(n), 'start':str(s), 'q':q})
...
class CrawlerThread(threading.Thread):
    def __init__(self, gui, new_search_event):
        ...
        self.result_per_batch = 10    # This specifies how many URLs are returned each time we query Google Search
    ...
    def run(self):
        while not self.quit_flag:   # This loop is not part of the crawling procss, it is just used to restart search on a new query as run() can only be called once
            if not self.new_search_event.isSet():
                time.sleep(1)
                continue
            ...
            threshold = self.gui.max_number_of_result + 2 - self.result_per_batch    # This sets the stopping condition when the number of URLs visited reaches the maximum number of result specified by the user
            current_batch_index = 1    # This specifies which batch of result we want from Google making sure we dont get the same part more than once
            ...
            while not self.stop_flag:    # We keep going until all tasks are completed as long as the user do not terminate us in the middle of the crawling process
                if current_batch_index < threshold:    # If the number of Google results (in terms of batches) do not exceed the maximum number specified by the user
                    try:
                        batch = jsonFaroo(self.result_per_batch, current_batch_index, self.gui.query)    # Try to get a batch of result from Google
            ...
            end_time = time.time()    # Stop the timer and see how long the crawl process took
...
class ProcessingThread(threading.Thread):
    ...
    def run(self):
        while not self.quit_flag:
            if not self.result_changed_event.isSet():
                time.sleep(1)
                continue
            self.result_changed_event.clear()
            self.gui.coverage_box.delete(*self.gui.coverage_box.get_children());
...
#            informative = informativeness(result, self.gui.inverse_doc_frequency)
            uniques = set([self.gui.sorted_inverse_result[synset_index][0] for synset_index in list(self.gui.sorted_inverse_result.keys())])
            duplicates = set(result.keys()).difference(uniques)

            # For populating parents
            remap_duplicate_to_unique = dict()
            unique_to_duplicate = dict([(u, findNearestDuplicate(result, self.gui.inverse_doc_frequency, list(duplicates), u)) for u in uniques])
            for u in unique_to_duplicate:
                d = unique_to_duplicate[u]
                if d in remap_duplicate_to_unique:
                    remap_duplicate_to_unique[d].append(u)
                else:
                    remap_duplicate_to_unique[d] = [u]

            # For populating children
            remap_unique_to_duplicate = dict()
            duplicate_to_unique = dict([(d, findNearestUnique(result, self.gui.inverse_doc_frequency, list(uniques), d)) for d in duplicates])
            for d in duplicate_to_unique:
                u = duplicate_to_unique[d]
                if u in remap_unique_to_duplicate:
                    remap_unique_to_duplicate[u].append(d)
                else:
                    remap_unique_to_duplicate[u] = [d]
            
            for d in remap_duplicate_to_unique:
                for u in remap_duplicate_to_unique[d]:
                    parent = self.gui.coverage_box.insert("", "end", text=u, tags=('parent',))
                    if u in remap_unique_to_duplicate:
                        for d in remap_unique_to_duplicate[u]:
                            self.gui.coverage_box.insert(parent, "end", text=d, tags=('child',))
                self.gui.coverage_box.insert("", "end", text="", tags=('empty',))
            self.gui.coverage_box.tag_configure('parent', foreground='red')
            self.gui.coverage_box.tag_configure('child', foreground='blue')
        return

    def quitThread(self):
        self.quit_flag = True

def findNearestDuplicate(result, inverse_doc_frequency, duplicates, u):
    scores = [similarity(result, inverse_doc_frequency, u, d) for d in duplicates]
    idx = scores.index(max(scores))
    return duplicates[idx]

def findNearestUnique(result, inverse_doc_frequency, uniques, d):
    scores = [similarity(result, inverse_doc_frequency, u, d) for u in uniques]
    idx = scores.index(max(scores))
    return uniques[idx]

def similarity(result, inverse_doc_frequency, k1, k2):
    denominator = sum([inverse_doc_frequency[synset_index] for synset_index in result[k1]])
    if denominator == 0:
        return 0
    else:
        numerator = sum([inverse_doc_frequency[synset_index] for synset_index in set(result[k1]).intersection(set(result[k2]))])
...
def inverseResult(result):
    ...
    for synset_index in inverse_result:
...
class GUI:
    def __init__(self, parent):
        self.parent = parent
        self.parent.wm_protocol("WM_DELETE_WINDOW", self.quitProgram)
        
        self.menubar = Menu(parent)
        self.menubar.add_command(label="START", command=self.startSearch)
        self.menubar.add_command(label="HELP", command=self.helpMessage)
        self.menubar.add_command(label="STOP", command=self.stopSearch)
        self.menubar.add_command(label="ABOUT", command=self.aboutMessage)
        ...
        self.main_window.bind_all("<Double-Button-1>", self.mouse_button_1_handler)
        self.main_window.bind_all("<Control-Button-1>", self.ctrl_mouse_button_1_handler)
        ...
        self.coverage_box = Treeview(self.coverage_window, selectmode=EXTENDED, yscrollcommand=self.coverage_scroll.set, name="coverage_box")
        self.coverage_box['show'] = 'tree'
    ...
    def mouse_button_1_handler(self, event):
        ...
        if "status_box" in mouse_pointer_address:
            pass
        if "coverage_box" in mouse_pointer_address and (len(self.coverage_box.get_children()) > 0):
            selections = self.coverage_box.selection()
            if len(selections) == 1:
                url = self.coverage_box.item(selections[0], "text")
                if len(url) > 0:
                    self.openURL(url)
    ...
    def ctrl_mouse_button_1_handler(self, event):
        ...
        if "status_box" in mouse_pointer_address:
            pass
        if "coverage_box" in mouse_pointer_address and not self.new_search_event.isSet() and (len(self.coverage_box.get_children()) > 0):
            selections = self.coverage_box.selection()
            coverage = set()
            for item in selections:
                url = self.coverage_box.item(item, "text")
                coverage.update(self.result[url])
            coverage_percent = 0
            ...
            self.putStatus('Your selections cover ' + str(coverage_percent) + '% of all information in the first ' + str(len(self.result)) + ' search results')
    ...
    def quitProgram(self):
        if messagebox.askokcancel("Quit", "Do you want to quit?"):
            ...
            if not self.crawler_thread.isAlive() and not self.processing_thread.isAlive():
                self.parent.destroy()
        pass

    def aboutMessage(self):
        messagebox.showinfo("About",
                            "NetTotal by Evan Tam 2015\n\n" +
                            "Email: tamevan@gmail.com\n\n" +
                            "NetTotal uses the following technologies:\n" +
                            "Python 3.4\nNLTK 3.0\nNumPy 1.9\nBeautiful Soup 4.3"
                            )

    def helpMessage(self):
        messagebox.showinfo("Help",
                            "Step 1: Type/paste your query into the box at the top left, your query can be words, sentences or even whole paragraphs if you wish to be more specific about your search.\n\n" +
                            "Step 2: Click 'START' and wait for the processing of your query, you can see the program status at the bottom left box, you may stop at any time by clicking 'STOP'. While the program is processing you can left-click and open the intermediate results in the box on the right\n\n" +
                            "Step 3: After processing is completed, the bottom left box shows the time taken. Your final result is shown on the right, your search summary consist of the links in red and the recommended browsing order is shown by the numbers in the beginning of the links starting from 0. The links in black are duplicates and should be ignored as reading them will not provide you will extra information.\n\n" +
                            "Step 4: If you hold the 'Ctrl' key, left-click and select multiple links, NetTotal will estimate how much new information you will get from reading your selection, this information is shown in the box at the bottom left.\n\n" +
                            "Step 5: Reading the pages from all red links is the same as reading all the pages examined by NetTotal. As there is limited information on the internet related to your search query, the rate of increase in red links will decrease exponentially as more results are examined by NetTotal because most of the later results are duplicates."
                            )

def main():
    root = Tk()
    root.wm_title("NetTotal")
    gui = GUI(root)
    root.mainloop()
    return
