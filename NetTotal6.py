...
class GUI:
    ...
    def mouse_button_1_handler(self, event):
        ...
        if "status_box" in mouse_pointer_address:
            pass
        if "coverage_box" in mouse_pointer_address and (self.coverage_box.size() > 0):
    ...
    def ctrl_mouse_button_1_handler(self, event):
        ...
        if "status_box" in mouse_pointer_address:
            pass
        if "coverage_box" in mouse_pointer_address and not self.new_search_event.isSet() and (self.coverage_box.size() > 0):
            highlighted_index = self.coverage_box.curselection()
            url_list = list(self.result.keys())
            coverage = set()
