...
class CrawlerThread(threading.Thread):
    ...
    def run(self):
        while self.new_search_event.wait():   # This loop is not part of the crawling procss, it is just used to restart search on a new query as run() can only be called once
...
class ProcessingThread(threading.Thread):
    ...
    def run(self):
        while self.result_changed_event.wait():
...
def resultSimilarity(result, targets, inverse_doc_frequency):
    column = dict()
    for outer in targets:
        if result[outer]:
            similar_tuples = [(similarity(result, outer, inner, inverse_doc_frequency), inner) for inner in targets if not (inner == outer)]
            column[outer] = sorted(similar_tuples, key=lambda u: u[0], reverse=True)
    return column

def similarity(result, k1, k2, inverse_doc_frequency):
    denominator = sum([inverse_doc_frequency[synset_index] for synset_index in set(result[k1])])
    if not result[k1] or not result[k2] or denominator == 0:
        return 0
    else:
...
class GUI:
    def __init__(self, parent):
        ...
        self.main_window = PanedWindow(parent, orient=VERTICAL, name="main_window")
        ...
        self.search_window = PanedWindow(self.main_window, orient=HORIZONTAL, name="search_window")
        ...
        self.result_window = PanedWindow(self.main_window, orient=HORIZONTAL, name="result_window")
        self.main_window.add(self.result_window)

        self.coverage_window = Frame(self.result_window, name="coverage_window")
        self.result_window.add(self.coverage_window)
        ...
        self.similar_window = Frame(self.result_window, name="similar_window")
        self.result_window.add(self.similar_window)
        self.similar_scroll = Scrollbar(self.similar_window)
        self.similar_scroll.pack(side=RIGHT, fill=Y)
        self.similar_box = Listbox(self.similar_window, yscrollcommand=self.similar_scroll.set, name="similar_box")
        self.similar_scroll.config(command=self.similar_box.yview)
        self.similar_box.pack(side=LEFT, fill=BOTH, expand=1)
    ...
    def putCoverage(self, items):
        self.coverage_box.delete(0, END)
        for item in items:
            self.coverage_box.insert(END, item)

    def putSimilar(self, items):
        self.similar_box.delete(0, END)
        for item in items:
            self.similar_box.insert(END, item)
    ...
    def mouse_button_1_handler(self, event):
        ...
        if "coverage_box" in mouse_pointer_address:
            ...
            if len(highlighted_index) == 1:
                ...
                self.openURL(url)
        if "similar_box" in mouse_pointer_address:
            pass
