...
class ProcessingThread(threading.Thread):
    ...
    def run(self):
        while not self.quit_flag:
            ...
            self.gui.sorted_summary = collections.OrderedDict(sorted([(url, informative[url]) for url in unsorted_summary], key=lambda u: u[1], reverse=True))
            duplicates = collections.OrderedDict()
            summary_index_to_url = list(self.gui.sorted_summary.keys())
            for url in self.gui.result:
                if url not in self.gui.sorted_summary:
                    duplicates[url] = None
                    parent = findNearestUrlInTargetList(result, self.gui.inverse_doc_frequency, summary_index_to_url, url)
                    if isinstance(self.gui.sorted_summary[parent], list):
                        self.gui.sorted_summary[parent].append(url)
                    else:
                        self.gui.sorted_summary[parent] = [url]
            duplicate_list = list(duplicates.keys())
            for url in self.gui.sorted_summary:
                parent = findNearestUrlInTargetList(result, self.gui.inverse_doc_frequency, duplicate_list, url)
                if duplicates[parent] is None:
                    duplicates[parent] = [url]
                else:
                    duplicates[parent].append(url)
            idx = 0
            for d in duplicates:
                if duplicates[d] is not None:
                    for url in duplicates[d]:
                        parent = self.gui.coverage_box.insert("", "end", text=url, tags=('parent',))
                        if isinstance(self.gui.sorted_summary[url], list):
                            for duplicate in self.gui.sorted_summary[url]:
                                self.gui.coverage_box.insert(parent, "end", text=duplicate, tags=('child',))
...
def findNearestUrlInTargetList(result, inverse_doc_frequency, target_list, url):
    scores = [similarity(result, inverse_doc_frequency, target, url) for target in target_list]
...
class GUI:
    ...
    def ctrl_mouse_button_1_handler(self, event):
        ...
        if "coverage_box" in mouse_pointer_address and not self.new_search_event.isSet() and (len(self.coverage_box.get_children()) > 0):
            ...
            self.putStatus('Your selections cover ' + str(coverage_percent) + '% of all information in the first ' + str(len(self.result)) + ' Google Search results')
