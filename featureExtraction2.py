...
def googleSearch(max_result, query):
    google_result = dict()
    result_per_batch = 8
    for batch_index in range(round(max_result / result_per_batch)):
        batch_result = ajaxGoogle(result_per_batch, batch_index * result_per_batch, query)
        google_result.update(batch_result)
    return google_result

def ajaxGoogle(n, i, q):
    query = urlencode({'rsz':str(n), 'start':str(i), 'q':q})
    ...
    while True:
        ...
        if json['responseData']:
            ...
            break
        time.sleep(10)
    return dict([(result['url'], []) for result in results])

def successfulUpdate(lancaster, stemmed_query, google_result, url, index_to_synset, noun_to_synset_index, verb_to_synset_index, subsumer_cache):
    try:
        ...
        google_result[url] = noun_arc.union(verb_arc)
        return True
